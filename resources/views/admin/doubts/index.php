<div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3 mb-4">
    <div>
        <h4 class="fw-bold mb-1">Dúvidas dos Alunos na Academia</h4>
        <p class="text-muted small mb-0">Responda às dúvidas enviadas pelos estagiários durante as aulas na Zona de Estudo.</p>
    </div>
    <div class="d-flex gap-2">
        <a href="/admin/doubts?filter=all" class="btn <?= $filter === 'all' ? 'btn-primary' : 'btn-outline-secondary' ?> btn-sm">
            Todas (<?= count($doubts) ?>)
        </a>
        <a href="/admin/doubts?filter=pending" class="btn <?= $filter === 'pending' ? 'btn-warning text-dark fw-bold' : 'btn-outline-warning text-dark' ?> btn-sm">
            <i class="bi bi-hourglass-split me-1"></i> Pendentes de Resposta
        </a>
        <a href="/admin/doubts?filter=answered" class="btn <?= $filter === 'answered' ? 'btn-success' : 'btn-outline-success' ?> btn-sm">
                        ...
                        <?php if ($isAnswered): ?>
                            <div class="p-3 bg-success bg-opacity-10 border border-success border-opacity-25 rounded-3 text-dark small">
                                <div class="d-flex justify-content-between align-items-center mb-1">
                                    <strong class="text-success"><i class="bi bi-check-circle-fill me-1"></i> Resposta Oficial (por <?= \App\Helpers\e($d['answered_by_name'] ?? 'Administrador') ?>):</strong>
...
function openReplyModal(doubtId, internName, questionText) {
    document.getElementById('formReplyDoubt').action = `/admin/doubts/${doubtId}/answer`;
